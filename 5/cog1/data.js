...
define(["exports", "data", "glMatrix"], function(data, exports) {
	...
	function vectorToKey(v) {
		return `${v[0]},${v[1]},${v[2]}`;
	  }

	function updatePoly(oldIdx, newIdx, vertices) {
	for (var i = 0; i < vertices.length; i++) {
		for (var j = 0; j < 3; j++) {
		try {
			if (vertices[i][j] === oldIdx) {
			vertices[i][j] = newIdx;
			}
		} catch {
			debugger;
	 ...
	 */
	 function cleanData() {
		...
		var doublePolyVerticeIndices = {};
		var doubleVectors = {};
		//clean double vectors
		for (var i = 0; i < this.vertices.length; i++) {
		  var vertexToCheck = this.vertices[i];
		  if (doubleVectors[vectorToKey(vertexToCheck)]) {
			continue; //been there done that
		  }
		  //check starting from the next
		  for (var j = i + 1; j < this.vertices.length; j++) {
			var next = this.vertices[j];
	
			if (vectorsEqual(vertexToCheck, next)) {
			  if (
				!doubleVectors[vectorToKey(vertexToCheck)] ||
				!doubleVectors[vectorToKey(vertexToCheck)].found
			  ) {
				var tmp = [];
				tmp.push(j);
				doubleVectors[vectorToKey(vertexToCheck)] = {};
				doubleVectors[vectorToKey(vertexToCheck)].original = i;
				doubleVectors[vectorToKey(vertexToCheck)].found = tmp;
			  } else {
				doubleVectors[vectorToKey(vertexToCheck)].found.push(j);
			  }
			}
		  }
		}
		console.log(doubleVectors);
		Object.values(doubleVectors).forEach((x) => {
		  x.found.forEach((y) => {
			this.polygonVertices = updatePoly(y, x.original, this.polygonVertices);
		  });
		});
	
		//clean double polys
		for (var i = 0; i < this.polygonVertices.length; i++) {
		  var vertexToCheck = this.polygonVertices[i];
		  if (doublePolyVerticeIndices[vectorToKey(vertexToCheck)]) {
			continue; //been there done that
		  }
		  //check starting from the next
		  for (var j = i + 1; j < this.polygonVertices.length; j++) {
			var next = this.polygonVertices[j];
	
			if (vectorsEqual(vertexToCheck, next)) {
			  if (
				!doublePolyVerticeIndices[vectorToKey(vertexToCheck)] ||
				!doublePolyVerticeIndices[vectorToKey(vertexToCheck)].found
			  ) {
				var tmp = [];
				tmp.push(j);
				doublePolyVerticeIndices[vectorToKey(vertexToCheck)] = {};
				doublePolyVerticeIndices[vectorToKey(vertexToCheck)].found = tmp;
			  } else {
				doublePolyVerticeIndices[vectorToKey(vertexToCheck)].found.push(j);
			  }
			}
		  }
		}
		...
		var indicesToDelete = [];
		Object.values(doublePolyVerticeIndices).forEach((x) => {
		  x.found.forEach((y) => {
			indicesToDelete.push(y);
		  });
		});
		for (var i = indicesToDelete.length - 1; i >= 0; i--) {
		  this.polygonVertices.splice(indicesToDelete[i], 1);
		}
